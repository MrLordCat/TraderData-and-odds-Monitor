...
class GameRenderer {
  ...
  render(data) {
    ...
    this.drawTerrain(data.terrain, data.terrainTypes);
    this.drawGrid();
    this.drawPath(data.waypoints, data.pathCells);
    this.drawSpecialElements(data.energyNodes, data.resourceVeins, data.terrainTypes);
  ...
  /**
   * Draw terrain layer (world coordinates)
   * Renders different terrain types with their colors
   */
  drawTerrain(terrain, terrainTypes) {
    ...
    for (let gy = startGridY; gy <= endGridY; gy++) {
      ...
      for (let gx = startGridX; gx <= endGridX; gx++) {
        ...
        this._drawTerrainDecoration(ctx, terrainType, x, y, gridSize);
      }
    }
  }
   ...
   */
  _drawTerrainDecoration(ctx, terrainType, x, y, size) {
    const centerX = x + size / 2;
    const centerY = y + size / 2;
    
    switch (terrainType) {
      case 'hill':
        // Draw hill pattern (triangular)
  ...
  /**
   * Draw special elements (energy nodes, resource veins) with glow effects
   */
  drawSpecialElements(energyNodes, resourceVeins, terrainTypes) {
    const ctx = this.ctx;
    const gridSize = CONFIG.GRID_SIZE;
    ...
    if (resourceVeins && terrainTypes.resource_vein) {
      ...
      resourceVeins.forEach(vein => {
        ...
        // Shimmering effect
        const shimmer = 0.5 + 0.5 * Math.sin(this.frameCount * 0.03 + vein.x);
