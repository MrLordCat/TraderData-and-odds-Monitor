      ...
      if (idx < activeBrokers.length) {
        ...
        if (!views[slotId]) {
          ...
          const slotView = new BrowserView({
            webPreferences: {
              preload: path.join(__dirname, '..','..','preloads','slot.js'),
          ...
          try { slotView.webContents.loadFile(path.join(__dirname,'..','..','..','renderer','pages','slot.html'), { query: { slot: String(idx), preset: presetId } }); } catch (e) {}
        }
