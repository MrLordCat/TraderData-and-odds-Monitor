...
function createBrokerManager(ctx){
  ...
  const view = new BrowserView({ webPreferences:{ preload: path.join(__dirname,'..','..','preloads','broker.js'), partition:'persist:'+brokerDef.id, nodeIntegration:false, contextIsolation:true, sandbox:false, javascript:true, backgroundThrottling:false } });
  ...
  try { if(ctx.statsManager && ctx.statsManager.ensureTopmost) ctx.statsManager.ensureTopmost(); } catch(_){ }
    ...
    view.webContents.on('did-fail-load', (e, errorCode, errorDesc, validatedURL, isMainFrame)=>{
      ...
      try { view.webContents.loadFile(path.join(__dirname,'..','..','..','renderer','pages','error.html'), { query:{ bid:brokerDef.id, code:String(errorCode), msg:safeMsg, target: validatedURL||'' } }); } catch(err){ }
    });
