/**
 * Entry point for index.html (main window)
 * Handles stage bounds, splitter, and CSS hot reload
 */

// Stage bounds computation
function computeStage() {
  ...
  window.desktopAPI.setStageBounds({
    x: 0,
    y: Math.round(top),
...
let __stageRaf = null;
let __lastStageReq = 0;
const __STAGE_INTERVAL = 60; // ms

function queueStage() {
  const now = Date.now();
  if (now - __lastStageReq < __STAGE_INTERVAL) {
    if (!__stageRaf) {
      __stageRaf = requestAnimationFrame(() => {
        __stageRaf = null;
        computeStage();
      });
    }
...
// Listen for panel state updates
