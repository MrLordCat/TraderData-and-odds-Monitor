...
function setMax(body, h) {
  if (!body) return;
  body.style.maxHeight = (h == null ? '0' : h + 'px');
}

function dispatchLayoutEvent() {
  setTimeout(() => {
    try { window.dispatchEvent(new Event('lol-table-layout-changed')); } catch (_) { }
  }, 180);
}
...
function apply(root, id, animate) {
  ...
  if (collapsed) {
    ...
    if (animate) {
      ...
      body.style.opacity = '1';
      setTimeout(() => {
        if (!state[id]) body.style.maxHeight = 'none';
        dispatchLayoutEvent();
      }, 180);
    }
  }
...
function bind(root) {
  if (root.dataset.collapseBound) return;
  root.dataset.collapseBound = '1';
  ...
  // Optional: toggle by clicking the section title
  try {
  ...
  if (typeof window !== 'undefined') {
    window.addEventListener('resize', () => {
      if (!state[id]) {
        const wasAuto = body.style.maxHeight === 'none';
        if (!wasAuto) {
          const h = measure(body);
          setMax(body, h);
        }
      }
    });
...
export function init() {
  document.querySelectorAll('.collapsible').forEach(bind);
}

export function toggle(id, forceState) {
  if (forceState !== undefined) state[id] = forceState;
  else state[id] = !state[id];
  const root = document.getElementById(id);
  if (root) apply(root, id, true);
  persist();
}
...
const StatsCollapse = { init, toggle, getState };
