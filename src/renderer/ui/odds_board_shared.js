...
// Legacy buildRowsHtml for initial render or full rebuild
function buildRowsHtml(records, opts){
  ...
  const html = (records||[]).map(r=>{
    ...
    previousOdds.set(broker, { odds1: o1, odds2: o2 });

    if(variant === 'embedded'){
      const swapBtn = `<button class="eo-swapBtn ${swappedOn?'on':''}" data-broker="${broker}" title="Swap sides">⇄</button>`;
      return `<tr data-broker="${broker}" class="${frozenCls}">`+
        `<td class="eo-broker"><span class="eo-broker-label" title="${r.frozen?'Suspended / stale':''}">${broker}</span></td>`+
        `<td class="odds-cell-1 ${bestCls1} ${frozenCls}">${d1}</td>`+
        `<td class="eo-swap-cell">${swapBtn}</td>`+
        `<td class="odds-cell-2 ${bestCls2} ${frozenCls}">${d2}</td>`+
      `</tr>`;
    }

    // default: board
    return `<tr data-broker="${broker}" class="${frozenCls}"><td><div class="brokerCell"><span class="bName eo-broker-label" title="${broker}">${broker}</span><button class="swapBtn ${swappedOn?'on':''}" data-broker="${broker}" title="Swap sides">⇄</button></div></td>`+
           `<td class="odds-cell-1 ${bestCls1} ${frozenCls}">${d1}</td>`+
           `<td class="odds-cell-2 ${bestCls2} ${frozenCls}">${d2}</td></tr>`;
  }).join('');
